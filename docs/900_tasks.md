...
- [ ] **未使用propsの削除** (PR#17)
  - `CommentControlPanelProps`の`videoId`が表示のみで未使用
  - 将来使用予定がなければ削除を検討

- [ ] **ポート番号のハードコード解消** (PR#16)
  - 19800/19801が複数箇所にハードコード
  - 定数または設定から取得するよう変更

- [ ] **Default trait実装** (PR#6)
  - `WebSocketState::new()`に対してClippyが`Default`実装を推奨
...
- [ ] **メッセージ詳細情報の取得** (PR#7)
  - `sticker_id`: スーパーステッカーの詳細
  - `level`: メンバーシップレベル
  - `count`: ギフト数
  - YouTube APIレスポンスに含まれる情報を活用
...
- [ ] **テストコメントの視覚的区別** (PR#16)
  - テストコメントにフラグを追加してUIで区別可能に

### アクセシビリティ

- [ ] **キーボード操作対応** (PR#16)
  - `role="button"` と `tabIndex={0}` を追加
  - `onKeyDown`でEnter/Spaceキー処理
